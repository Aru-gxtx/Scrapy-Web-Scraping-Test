...
def build_payload(item):
    ...
    # Use product_name as fallback if overview is empty
    if not overview:
    ...
    # Build features from vendor and price if available
    features = " | ".join(part for part in [vendor, price] if part)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Populate empty cells in STEELITE.xlsx using russoequip scraped data, "
    ...
    parser.add_argument(
        "--json",
        default="steelite/russoequip.json",
        help="Path to russoequip JSON file",
    )
