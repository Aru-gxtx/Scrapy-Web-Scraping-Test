...
def build_payload(item):
    ...
    # On BGBenton pages, SKU is typically the product barcode/EAN.
    sku = clean_value(item.get("sku"))
    barcode = clean_value(item.get("barcode")) or sku

    payload = {
        "Image Link": clean_value(item.get("image_link")),
        "Overview": overview,
        "Length": clean_value(item.get("length")),
        "Width": clean_value(item.get("width")),
        "Height": clean_value(item.get("height")),
        "Capacity": clean_value(item.get("volume")),
        "Features": "",
        "Edge Style": "",
        ...
        "EAN Code": clean_value(item.get("ean_code")) or sku,
        "Barcode": barcode,
        # Optional columns: written only if these headers exist in workbook.
        "Price": clean_value(item.get("price")),
        "Product URL": clean_value(item.get("product_url")),
        "Search URL": clean_value(item.get("search_url")),
    }
...
def populate_workbook(excel_path, json_entries):
    ...
    for json_item in json_entries:
        ...
        if json_catalog:
            # Apply one JSON entry to all rows that share this catalog in the workbook.
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Populate empty cells in STEELITE.xlsx using bgbenton scraped data, "
    ...
    parser.add_argument(
        "--json",
        default="steelite/bgbenton.json",
        help="Path to bgbenton JSON file",
    )
